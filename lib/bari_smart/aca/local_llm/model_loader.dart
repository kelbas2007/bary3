...
class ModelLoader {
  ...
  Future<String> loadModel({
    ...
    if (!forceReload && _loadedModelPath != null && _loadedVersion == version) {
      if (kDebugMode) {
        debugPrint(
          '[ModelLoader] Model v$version already loaded: $_loadedModelPath',
        );
      }
    ...
    if (kDebugMode) {
      ...
      debugPrint(
        '[ModelLoader] Model: ${metadata.modelName}, Size: ${metadata.expectedSize} bytes',
      );
    }
    ...
    try {
      ...
      if (await localFile.exists() && !forceReload) {
        if (kDebugMode) {
          debugPrint(
            '[ModelLoader] Model already exists locally: $localModelPath',
          );
        }
      ...
      try {
        ...
        if (kDebugMode) {
          ...
          debugPrint(
            '[ModelLoader] Model loaded successfully: $localModelPath ($fileSize bytes)',
          );
          debugPrint(
            '[ModelLoader] Model: ${metadata.modelName}, Quantization: ${metadata.quantization}',
          );
        }
        ...
        if (kDebugMode) {
          debugPrint('[ModelLoader] Error loading model from assets: $e');
        }
        // Если не удалось загрузить модель из assets, создаем пустой файл
        // чтобы избежать ошибок в других частях приложения
        if (kDebugMode) {
          debugPrint('[ModelLoader] Creating empty placeholder model file');
        }
