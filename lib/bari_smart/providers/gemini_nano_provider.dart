...
/// Провайдер для Gemini Nano (ML Kit GenAI) - бесплатный on-device AI
/// 
/// Поддерживает 3 языка: ru, en, de
/// Работает полностью офлайн после скачивания модели
class GeminiNanoProvider implements BariProvider {
...
''',
  ...
  Future<BariResponse?> tryRespond(
    ...
    if (!await _service.checkAvailability()) {
      if (kDebugMode) {
        debugPrint('[GeminiNano] Not available on this device');
      }
      return null;
    }

    // Проверяем, скачана ли модель
    if (!await _service.checkDownloaded()) {
      if (kDebugMode) {
        debugPrint('[GeminiNano] Model not downloaded');
      }
      return null;
    }
    ...
    if (!_sessionInitialized) {
      ...
      final initialized = await _service.initializeSession(systemPrompt);
      if (!initialized) {
        if (kDebugMode) {
          debugPrint('[GeminiNano] Failed to initialize session');
        }
        return null;
      }
    ...
    try {
      ...
      final response = await _service.generateResponse(promptWithHistory, locale);
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Error generating response: $e');
      }
      return null;
    }
  }
  ...
  /// Парсит JSON ответ от Gemini Nano
  BariResponse _parseResponse(String response, String locale) {
    try {
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] JSON parse error: $e');
        debugPrint('[GeminiNano] Raw response: $response');
      }
  ...
  Future<List<QuizSlide>?> generateQuizQuestions(
    ...
    if (!await _service.checkAvailability()) {
      if (kDebugMode) {
        debugPrint('[GeminiNano] Not available for quiz generation');
      }
      return null;
    }
    ...
    if (!await _service.checkDownloaded()) {
      if (kDebugMode) {
        debugPrint('[GeminiNano] Model not downloaded for quiz generation');
      }
      return null;
    }
    ...
    try {
      ...
      final response = await _service.generateResponse(prompt, locale);
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Error generating quiz: $e');
      }
      return null;
    }
  }
...
''',
  ...
  List<QuizSlide>? _parseQuizQuestions(String response, String locale) {
    try {
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Quiz parse error: $e');
        debugPrint('[GeminiNano] Raw response: $response');
      }
      return null;
    }
  ...
  Future<SpendingAnalysis?> analyzeSpending(
    ...
    // Проверяем доступность
    if (!await _service.checkAvailability() || !await _service.checkDownloaded()) {
      return null;
    }
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Error analyzing spending: $e');
      }
      return null;
    }
  }
...
''',
  ...
  SpendingAnalysis? _parseSpendingAnalysis(String response, String locale) {
    try {
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Spending analysis parse error: $e');
        debugPrint('[GeminiNano] Raw response: $response');
      }
      return null;
    }
  ...
  Future<String?> generateParentSummary(
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('[GeminiNano] Error generating parent summary: $e');
      }
      return null;
    }
  }
